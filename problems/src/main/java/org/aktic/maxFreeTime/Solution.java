...
public class Solution {
    ...
    private static int countMax(int[] startTime, int[] endTime) {
        int max = startTime[0];
        int left = 1;
        int right = 0;

        while (left < startTime.length) {
            max = Math.max(max, startTime[left] - endTime[right]);
            left++;
            right++;
        }
